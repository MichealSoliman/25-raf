...
//                   <h4 className="font-bold text-2xl text-[#2c2c2c] mb-4">مكونات الشقة:</h4>
...
// import { Button } from "@/components/ui/button"
// import { Badge } from "@/components/ui/badge"
// import { Bed, Bath, Square, Home, User, Users, Coffee } from "lucide-react"
// import Image from "next/image"
// import { useSourceTracking } from "@/hooks/use-source-tracking"
// import { generateWhatsAppUrl } from "@/lib/whatsapp-messages"
// import apiService from "@/lib/api-service"
...
//   "غرفة استقبال": <Home className="w-5 h-5 text-[#c48765]" />,
//   "2 غرفة نوم": <Bed className="w-5 h-5 text-[#c48765]" />,
//   "4 دورات مياه": <Bath className="w-5 h-5 text-[#c48765]" />,
//   "مجلس": <Users className="w-5 h-5 text-[#c48765]" />, // أيقونة للمجلس
//   "مجلس نساء": <Users className="w-5 h-5 text-[#c48765]" />,
//   "مطبخ": <Coffee className="w-5 h-5 text-[#c48765]" />,
//   "غرفة سائق": <User className="w-5 h-5 text-[#c48765]" />,
//   "غرفة خادمة": <User className="w-5 h-5 text-[#c48765]" />,
// }
...
export function ApartmentShowcase({ onBookingClick }: ApartmentShowcaseProps) {
  ...
  useEffect(() => {
    const loadApartmentsData = async () => {
      try {
        const apartmentsData = await apiService.getApartmentsData()
        setApartments(apartmentsData)
        if (apartmentsData.length > 0) setSelectedApartment(apartmentsData[0])
      } catch (error) {
        console.error("Error loading apartments data:", error)
        const defaultApartments = [
          {
            ...
            rooms: 4,
            bathrooms: 4,
            features: ["غرفة استقبال", "2 غرفة نوم", "3 دورات مياه", "مجلس نساء", "مطبخ"],
            popular: true,
            image: "/a.jpg",
          }
        ]
  ...
  if (!selectedApartment || apartments.length === 0) return null
            ...
            {apartments.map((apartment) => (
              ...
              >
                ...
                <div className="flex items-center space-x-4 space-x-reverse text-sm text-[#6b7280]">
                  ...
                  <div className="flex items-center space-x-1 space-x-reverse">
                    <span>{apartment.bathrooms} دورات مياه</span>
                  </div>
                  <div className="flex items-center space-x-1 space-x-reverse">
                    <span>{apartment.area} م²</span>
                  </div>
                ...
                <div className="mb-6 md:mb-8">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {selectedApartment.features.map((feature: string, index: number) => (
                      <div
                        key={index}
                        className="flex flex-col items-center bg-[#f5f3f0] rounded-xl p-4 shadow-sm hover:shadow-md transition-all"
                      >
                        <div className="w-12 h-12 mb-2 relative">
                          <Image
                            src={`/features/${feature}.jpeg`} // ضع الصور هنا في public/features
                            alt={feature}
                            fill
                            className="object-contain"
                          />
                        </div>
                        <span className="text-[#540f6b] font-bold text-x text-center">{feature}</span>
                      </div>
                    ))}
                  </div>
                </div>
